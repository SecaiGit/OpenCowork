...
import { isEditableUserMessage } from '../../image-attachments'
import { extractLatestUserInput } from '../../api/auto-model-selector'
import { sessionToMarkdown } from '../../utils/export-chat'
import { findOpenAIImagesPromptMessage } from '../../api/openai-images-provider'
import { isGeminiImageModelRequest } from '../../api/gemini-provider'
import { collectRecentRecommendationConversation } from '../../recommendation/prompt-recommendation'
...
describe('validateToolUseResultProtocol', () => {
  ...
  it.each([
    { name: 'post compact state', meta: { postCompactState: true as const } },
    { name: 'emergency shrink notice', meta: { contextEmergencyShrink: true } },
    {
    ...
    },
    {
      ...
      meta: {
        streamingContinuation: {
          ...
          stopReason: 'max_tokens',
          partialOutputChars: 512,
          continuationIndex: 1
        }
      }
    }
  ])('does not treat synthetic $name as user-authored input', ({ meta }) => {
    const realUser = message('user', 'continue the actual task')
    const synthetic = {
      ...message('user', 'internal context generated by compression'),
      meta
    }

    const analysis = buildTranscriptStaticAnalysis([realUser, synthetic])

    expect(isEditableUserMessage(synthetic)).toBe(false)
    expect(analysis.renderableMessageIds).toEqual([realUser.id])
    expect(analysis.lastRealUserMessageId).toBe(realUser.id)
  })

  it('renders compact summaries as compression output without making them user input', () => {
    nextMessageId = 300
    const realUser = message('user', 'continue the actual task')
    const summary = {
      ...message('user', '[Context Memory Compressed Summary]\n\nEarlier work summary'),
      meta: {
        compactSummary: {
    ...
    const analysis = buildTranscriptStaticAnalysis([realUser, summary])

    expect(isEditableUserMessage(summary)).toBe(false)
    expect(analysis.renderableMessageIds).toEqual([realUser.id, summary.id])
    expect(analysis.lastRealUserMessageId).toBe(realUser.id)
  })

  it('skips generated context messages when extracting latest user input for routing', () => {
    const realUser = message('user', 'route this real request')
    const toolOnlyUser = message('user', [toolResult('a', 'file contents')])
    const summary = {
      ...message('user', '[Context Memory Compressed Summary]\n\nGenerated summary'),
      meta: {
        compactSummary: {
    ...
    const emergencyNotice = {
      ...message('user', 'Earlier local context omitted'),
      meta: { contextEmergencyShrink: true }
    }

    expect(isUserAuthoredMessage(toolOnlyUser)).toBe(false)
    expect(extractLatestUserInput([realUser, toolOnlyUser, summary, emergencyNotice])).toBe(
      'route this real request'
    )
  })

  it('exports generated compact summaries as compression output instead of user input', () => {
    const realUser = message('user', 'actual request')
    const summary = {
      ...message('user', '[Context Memory Compressed Summary]\n\nGenerated summary'),
      meta: {
        compactSummary: {
    ...
    const postCompactState = {
      ...message('user', 'Generated working state'),
      meta: { postCompactState: true as const }
    }
    const assistant = message('assistant', 'assistant answer')

    const markdown = sessionToMarkdown({
      id: 's-1',
      title: 'Export test',
      mode: 'chat',
      messages: [realUser, summary, postCompactState, assistant],
      messageCount: 4,
      messagesLoaded: true,
      loadedRangeStart: 0,
      loadedRangeEnd: 4,
    ...
    expect(markdown).toContain('actual request')
    expect(markdown).toContain('## Context Compression')
    expect(markdown).toContain('Generated summary')
    expect(markdown).not.toContain('## User <sub>12:00 AM</sub>\n\nGenerated summary')
    expect(markdown).not.toContain('Generated working state')
  })

  it('uses the latest user-authored message as the OpenAI image prompt source', () => {
    const realUser = message('user', 'draw the actual request')
    const summary = {
      ...message('user', '[Context Memory Compressed Summary]\n\nGenerated summary'),
      meta: {
        compactSummary: {
    ...
    expect(findOpenAIImagesPromptMessage([realUser, summary, postCompactState])?.id).toBe(
      realUser.id
    )
  })

  it('uses the latest user-authored message when detecting Gemini image requests', () => {
    const realUserWithImage = message('user', [
    ...
    expect(
      isGeminiImageModelRequest([realUserWithImage, postCompactState], {
        ...providerConfig,
        type: 'gemini',
  ...
  it('does not label generated context messages as user text for prompt recommendations', () => {
    const realUser = message('user', 'actual recent request')
    const summary = {
      ...message('user', '[Context Memory Compressed Summary]\n\nGenerated summary'),
      meta: {
        compactSummary: {
    ...
    const assistant = message('assistant', 'assistant response')

    expect(
      collectRecentRecommendationConversation([realUser, summary, postCompactState, assistant])
    ).toEqual([
      { role: 'user', text: 'actual recent request' },
